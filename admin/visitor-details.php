...
class VisitorDetailsManager {
    ...
    public function updateVisitorStatus($vid, $status, $officialRemark) {
        $query = mysqli_query($this->con, "UPDATE tblvisitor SET officeRemark='$officialRemark', status='$status' WHERE id='$vid'");
        return $query;
    }

    public function getVisitorDetails($vid) {
        $query = mysqli_query($this->con, "SELECT * FROM tblvisitor WHERE id='$vid'");
        $visitorDetails = [];
...
if (strlen($_SESSION['aid']) == 0) {
    ...
    $visitorDetailsManager = new VisitorDetailsManager($con);

    if (isset($_POST['submit'])) {
        $vid = intval($_GET['vid']);
        $estatus = $_POST['status'];
        $oremark = $_POST['officialremak'];
        $queryResult = $visitorDetailsManager->updateVisitorStatus($vid, $estatus, $oremark);
    ...
    $vid = intval($_GET['vid']);
    $visitorDetails = $visitorDetailsManager->getVisitorDetails($vid);
?>
<!DOCTYPE html>
<html lang="en">
<head>
