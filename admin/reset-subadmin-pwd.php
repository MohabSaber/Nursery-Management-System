...
class SubAdminManager {
    ...
    public function resetSubAdminPassword($subAdminId, $newPassword) {
        $password = md5($newPassword);
        $query = mysqli_query($this->con, "UPDATE tbladmin SET Password='$password' WHERE UserType=0 AND ID='$subAdminId'");
        return $query;
    }
}
...
if(strlen($_SESSION['aid']) == 0) {
    ...
    if(isset($_POST['reset'])) {
        ...
        $newPassword = $_POST['inputpwd'];

        $subAdminManager = new SubAdminManager($con);
        $passwordReset = $subAdminManager->resetSubAdminPassword($subAdminId, $newPassword);
